...
def build_advice(models_table, kv):
    ...
    if profit_before_tax < 0:
        advice.append("Критическая рекомендация: остановить падение операционной рентабельности. Требуется жесткая оптимизация себестоимости продаж.")
        advice.append("Провести реструктуризацию долговых обязательств для снижения финансовой нагрузки.")
    
    if revenue / balance > 1.5 and profit_before_tax > 0:
        advice.append("Компания демонстрирует отличную динамику и масштабирование. Рекомендуется оптимизировать управление оборотным капиталом для устранения технического дефицита ликвидности.")
        advice.append("Текущая бизнес-модель эффективна, банкротство не прогнозируется.")
        ...
        if use_file:
            ...
            for _, row in df.iterrows():
                ...
                rows.append({
                    ...
                    "overall": overall_conclusion(models_table, kv)
                })
